...
\begin{algorithm}[H]{\textbf{cant}(ls: vint) $\to$ $res$ : Nat }
	\begin{algorithmic}
	\State aux $\gets$ 0
	\State Desde i = 0 hasta Tam(ls)-1
	\State \quad \textbf{if}(ls[i] != 0)
	\State \qquad aux $\gets$ aux+1
	\State \quad \textbf{fi}
	\State res $\gets$ aux
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{voyConA}(\textbf{In} VelElem1:Nat, \textbf{InOut} mMinActual:Nat, \textbf{InOut} ArqA: vint, \textbf{InOut} ArqB: vint)}
	\begin{algorithmic}
