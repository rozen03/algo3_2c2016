...
\subsection{Explicación de la solución}
   ...
   Para obtener el valor óptimo, modelamos nuestro problema a partir de la siguiente función recursiva:
	...
	%q pensas? se puede hablar de que la func rec es top down o eso es solo para el algoritmo?%
...
%$\prod_{i=1}^{3}K_{i}  = K_1*K_2*K_3$   sea $K_{max} = Max(K_1,K_2,K_3)$ y $K_o= K_1^3 + K_2^3 + K_3^3 - K_{max}^3$ entonces $K_1*K_2*K_3 < K_{max}^3 < K_{max}^3 + K_o = K_1^3+K_2^3+K_3^3 <(\sum_{i=1}^{3}K_{i})^{3}$ entonces $\prod_{i=1}^{3}K_{i}* T < \sum_{i=1}^{3}K_{i})^{3} * T $
