...
lema2 : probaremos por inducción que $ \Sum_{0}^{n}{k_{i}*3^{i}} < 3^{n}$ con $-1 \leq k_{i} \leq 1 $
%completar%
...
Probaremos por inducción global  que  $ \forall n \in N vale  \sum_{0}^{n}{a_{i}*3^{i}}= \sum_{0}^{n+1}{b_{i}*3^{i}} con 0\leq a_{i} \leq 2 y -1 \leq b_{i} \leq 1$ 
...
\begin{itemize}
	...
	\item Podemos pensar un arreglo de 4 dimensiones como un ''hipercubo" (el equivalente a un cubo pero en varias dimensiones, es particular en nuestro caso 4) 
	...
	\item Cuando se llama a LlenarMochilas desde GuardarTesoro cuesta $\mathcal{O}(T)$ ya que en las lineas 9,10,11 cuando se llama a ValorOptimo, el hipercubo objetoXpeso cuenta con todos los valores ya calculados entrando en el if (linea 6) que devuelve en $\mathcal{O}(1)$ el valor.
\end{itemize}

Volviendo a la demostración, tanto en el pseudocódigo como en la función matemática podemos ver que la recursión se realiza tantas veces como la cantidad total de tesoros, y las recursiones internas,las que dependen de cada mochila se realizan tantas veces como la capacidad de la misma. Para llenar un cubo, siempre debemos recurrir al cubo anterior, uno puede pensar que esto aportaría complejidad, sin embargo, al guardar estos valores sólo construimos estos cubos una vez, y la complejidad por acceso es $\Theta(1)$.
...
$\prod_{i=1}^{3}K_{i}  = K_1*K_2*K_3$   sea $K_{max} = Max(K_1,K_2,K_3)$ y $K_o= K_1^3 + K_2^3 + K_3^3 - K_{max}^3$ entonces $K_1*K_2*K_3 \leq K_m^3 \leq K_m^3 + K_o = K_1^3+K_2^3+K_3^3 <(\sum_{i=1}^{3}K_{i})^{3}$ entonces $\prod_{i=1}^{3}K_{i}* T < (\sum_{i=1}^{3}K_{i})^{3} * T $
