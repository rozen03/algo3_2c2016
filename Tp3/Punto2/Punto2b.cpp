...
//IDEA DISTINTA AL GOLOZO
/*
La idea es que en vez de mirar el gimnasio mas cerca, mires el gimnasio a vencer mas cercano, luego la pokeparada mas cercana, y vayas a la mas cercana de las dos.
Esto con algunos casos a considerar, como por ej si ir a una pokeparada te hace perder pociones, o si te hace perder pociones pero no tenes gimnasios que vencer.
...
void goloso(){
	...
	while(!Gimnasios.empty()){//mientras sigan existiendo gimnasios que no pasaron  
		...
		irPoke = PokeParadaMasCercana();
		
		if(irGim != 0 && irPoke != 0 ){
			if(dist(irGim)>dist(irPoke)){
				proxLugar = irPoke;
			}
			else {
				proxLugar = irGim;
