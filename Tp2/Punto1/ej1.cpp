...
int Solucion(vector<Nodo *>& nodos, vector<Eje *>& ejes, int p){
	...
	//que multiplicar por p para tener algo lo suficientemente grande pero por las dudas y para no pensar de mas...
	if(p == 0) res = 2*nodos.size();
	//como res va a ir cambiando creo una copia, si es igual no había solucion.
	int nores = res;
	for(int i=0; i<=p; i++){
