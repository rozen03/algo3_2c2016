...
void MostrarNodo(Nodo * nodo){
	cout<< nodo -> indice<<" ("<<nodo ->nivel<<") ";
	if(nodo ->esPared)cout<<"P ";
	if(nodo ->esFinal) cout<<"F ";
}

void MostrarVecino(Nodo * n){
	vector<Eje *> ejes = n ->ejejes;
	cout<<"[";
	for(int i = 0; i<ejes.size(); i++){
		Eje * ej = ejes[i];
		Nodo * nodAux = ej ->dameElOtroNodoPorfa(n);
		MostrarNodo(nodAux);
		cout<<", ";
	}
	cout<<"]";
...
void MostrarCopia(vector<Nodo *>  nodos){
	for(int i = 0; i< nodos.size(); i++){
		Nodo * aux = nodos[i];
		MostrarNodo(aux);
		cout<<": ";
