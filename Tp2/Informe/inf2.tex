...
\begin{algorithm}[H]{\textbf{solucion}(int n, vecor$<$ejes$>$ aristas)}
	\begin{algorithmic}[1]
		\State \quad convertir aristas en cola de prioridad según el peso \Comment $\Ogr(m)$
		\State \quad int res= Kruskal(aristas,n)  \Comment $Ogr(mxlog(n))$
		\State \quad devolver res \Comment \Ogr(1)
	\end{algorithmic}
\end{algorithm}
